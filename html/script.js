...
$(document).on('click', '#create', function(e){
    ...
    if (stringIsEnglish($('#first_name').val()) == true && stringIsEnglish($('#last_name').val()) == true && stringIsEnglish($('#nationality').val()) == true) {
        ...
        console.log('[qb-multicharacter] Tried to create an char with non-english name.')
    }
});
...
$("#close-reg").click(function (e) {
    ...
    $('.characters-list').css("filter", "none")
    saMultiCharacters.fadeOutDown('.character-register', '125%', 400);
    $.post('http://qb-multicharacter/getOffChar');
...
function select_character(id) {
    var chari = id
    $.post('http://qb-multicharacter/getCloserToCharacter', JSON.stringify({
        charid: chari
    }));
...
function update_char_marker(id) {
    var chari = id
    $.post('http://qb-multicharacter/updateCharMarker', JSON.stringify({
        charid: chari
    }));
};
